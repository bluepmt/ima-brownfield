...
(function () {
  ...
  /* sessionStorage keys — same origin as auth so auto-clears on tab close / logout */
  const STORAGE_HIST = 'ima_chat_hist_v1';
  const STORAGE_MSGS = 'ima_chat_msgs_v1';
  ...
  const KB = [
    ...
    {
      keys: ['progress', 'report', 'báo cáo', 'tiến độ', 'execution phase', 'tender', 'brain'],
      a: '<b>Progress Report</b> will be updated during the project execution phase.',
      link: { href: '08. Progress Report.html', text: 'Mở Progress Report →' }
    },
    {
  ...
  .ima-bot-msg.bot p > a:only-child:hover {
    ...
    text-decoration: none;
      ...
      .ima-bot-eq-link{display:inline;color:#24397A;font-weight:700;text-decoration:underline dotted;cursor:pointer;white-space:nowrap;}
      .ima-bot-eq-link:hover{color:#5B8FE8;text-decoration:underline solid;}
      .ima-bot-citations{margin-top:8px;display:flex;flex-wrap:wrap;gap:5px;}
      .ima-bot-cite-chip{display:inline-flex;align-items:center;gap:3px;padding:3px 9px;border-radius:12px;background:rgba(36,57,122,0.08);border:1px solid rgba(36,57,122,0.18);color:#24397A;font-size:10px;font-family:Georgia,serif;text-decoration:none;transition:background 0.15s;}
      .ima-bot-cite-chip:hover{background:rgba(36,57,122,0.15);border-color:#24397A;}
  ...
  function build() {
    ...
    /* Stream bot reply: type out plain text gradually, then swap to formatted HTML.
       ...
       onDone (optional): called when the animation completes (used to save session). */
    function addBotStream(finalHtml, onDone) {
      const div = document.createElement('div');
      div.className = 'ima-bot-msg bot';
      messages.appendChild(div);
      revealMsgTop(div);
    ...
    /* ── Session persistence (survives page nav, clears on tab close) ── */
    function saveSession() {
      try {
        sessionStorage.setItem(STORAGE_HIST, JSON.stringify(history));
        // Strip any streaming cursors before saving so restored HTML is clean
        var cleanHtml = messages.innerHTML.replace(/<span class="ima-bot-cursor"><\/span>/g, '');
        sessionStorage.setItem(STORAGE_MSGS, cleanHtml);
      } catch (e) { /* ignore quota errors */ }
    }

    function loadSession() {
      try {
        var h = sessionStorage.getItem(STORAGE_HIST);
        if (h) {
          var parsed = JSON.parse(h);
          parsed.forEach(function (t) { history.push(t); });
        }
        return sessionStorage.getItem(STORAGE_MSGS) || null;
      } catch (e) { return null; }
    }
    ...
    function answerOffline(q) {
      ...
      return 'I am temporarily unable to access the project knowledge base. For specific enquiries, please contact the <b>IMA Brownfield Management Team</b>, or browse the document portal: <a class="nav-btn" href="08. Progress Report.html">Progress Report</a>.';
    }
    ...
    /* renderCitations: post-processes bot HTML reply to:
       1. Auto-link equipment tags to trang 03 PFD
       2. Collect all cited pages and render citation chips at bottom */
    function renderCitations(html) {
      var EQ_TAGS = {
        'AA-8133': '03. Scope of Work.html',
        'V-8133':  '03. Scope of Work.html',
        'V-8151':  '03. Scope of Work.html',
        'C-8160':  '03. Scope of Work.html',
        'A-8160':  '03. Scope of Work.html',
        'A-8170':  '03. Scope of Work.html',
        'M-8101':  '03. Scope of Work.html',
        'M-8102':  '03. Scope of Work.html',
        'M-8103':  '03. Scope of Work.html',
        'P-8151':  '03. Scope of Work.html',
        'P-8152':  '03. Scope of Work.html',
        'FCV-332A':'03. Scope of Work.html',
        'FCV-332B':'03. Scope of Work.html',
        'FCV-332': '03. Scope of Work.html',
      };
      var PAGE_NAMES = {
        '01. Overview.html':                    '01 Overview',
        '02. Contract Strategies.html':         '02 Contract Strategies',
        '03. Scope of Work.html':               '03 Scope of Work / PFD',
        '04. Project Execution Plan.html':      '04 Execution Plan',
        '05. Manhours & Schedule.html':         '05 Schedule',
        '06. IMA Brownfield Project Team.html': '06 Project Team',
        '07. Images & Documents.html':          '07 Images & Documents',
        '08. Progress Report.html':             '08 Progress Report',
      };

      /* 1. Wrap equipment tags as clickable links.
            Only match tags NOT already inside an HTML tag (href, data-*, etc.) */
      Object.keys(EQ_TAGS).forEach(function (tag) {
        var escaped = tag.replace(/[-]/g, '\\-');
        var re = new RegExp('(?<!["\\/=])\\b(' + escaped + ')\\b(?![^<]*>)', 'g');
        var page = EQ_TAGS[tag];
        html = html.replace(re, function (m) {
          return '<a class="ima-bot-eq-link" href="' + page + '" title="View on PFD — ' + m + '">' + m + '</a>';
        });
      });

      /* 2. Extract all cited .html pages → render as chips at bottom */
      var cited = [];
      html.replace(/href="([^"#]+\.html)"/g, function (_, href) {
        if (PAGE_NAMES[href] && cited.indexOf(href) === -1) cited.push(href);
      });
      if (cited.length) {
        var chips = cited.map(function (href) {
          return '<a class="ima-bot-cite-chip" href="' + href + '">📄 ' + PAGE_NAMES[href] + ' →</a>';
        }).join('');
        html += '<div class="ima-bot-citations">' + chips + '</div>';
      }
    ...
    async function ask(q) {
      ...
      if (USE_WORKER) {
        try {
          ...
          addBotStream(renderCitations(mdToHtml(reply)), saveSession);
          return;
        } catch (err) {
      ...
      setTimeout(function () {
        typing.remove();
        fab.classList.remove('thinking');
        addBotStream(answerOffline(q), saveSession);
      }, delay);
    }

    /* Restore previous session or show welcome */
    var restoredMsgs = loadSession();
    if (restoredMsgs) {
      messages.innerHTML = restoredMsgs;
